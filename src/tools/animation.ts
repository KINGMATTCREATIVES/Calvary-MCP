...
export async function handleAnimationTools(
  name: string,
  args: any,
  ...
  if (name === "cavalry_playback_control") {
    ...
    let script = "";
    if (action === "play") script = "api.play();";
    else if (action === "stop") script = "if (typeof api.stop === 'function') api.stop();";
    else if (action === "rewind") script = "api.setFrame(0);";
